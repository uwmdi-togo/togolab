...
#'
#' Resolves which `bpt_paths.yml` to use, in priority order:
#' \enumerate{
#'   \item the `path` argument, if given;
#'   \item the `bptlab.config` R option (`getOption("bptlab.config")`);
#'   \item the `BPT_PATHS_CONFIG` environment variable;
#'   \item the copy shipped inside the installed package.
#' }
#'
#' Keeping a canonical `bpt_paths.yml` in the lab GitHub repo and pointing
#' `BPT_PATHS_CONFIG` at it (see the package README) means path edits take
...
#' bpt_config_path()
#' }
bpt_config_path <- function(path = NULL) {
  candidate <- path
  if (is.null(candidate)) candidate <- getOption("bptlab.config")
  if (is.null(candidate)) {
    env <- Sys.getenv("BPT_PATHS_CONFIG", unset = "")
    if (nzchar(env)) candidate <- env
  }
  if (is.null(candidate)) {
    candidate <- system.file("config", "bpt_paths.yml", package = "bptlab")
  }
  if (is.null(candidate) || !nzchar(candidate) || !file.exists(candidate)) {
    stop("Could not find a bpt_paths.yml config file. Looked at: ",
...
#' @param config Optional path to a YAML config (see [bpt_config_path()]).
#' @param setup_s3 If `TRUE` (default), also set the AWS/S3 environment
#'   variables via [bpt_setup_s3()] using this user's `keys` file.
...
bpt_paths <- function(user = Sys.info()[["user"]],
  ...
  cfg_file <- bpt_config_path(config)
